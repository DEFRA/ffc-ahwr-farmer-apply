...
module.exports = config.tenMonthRule.enabled === true
  ? [{
      method: 'GET',
      ...
      options: {
        ...
        handler: async (request, h) => {
          ...
          try {
            ...
            const businessCanApply = await businessEligibleToApplyWithDate(organisationSummary.organisation.sbi)
            reapplicationDate = businessCanApply?.dateCanReapply

            if (!businessCanApply.businessIsEligible) {
              throw new AlreadyAppliedError(`Business with SBI ${organisationSummary.organisation.sbi} is not eligible to apply`)
            }
