  ...
  describe("When 10 month rule toggle is enabled", () => {
    ...
    describe("When the previous application was within 10 months", () => {
      test.each([
        ...
        "Business is eligible when the last previous application had a status of WITHDRAWN (2) or NOT AGREED (7)",
      ...
      describe("Business is not eligible when the last previous application had a status of anything other than WITHDRAWN (2), NOT AGREED (7), AGREED (1)", () => {
        describe("Time limit error is returned with status of DATA INPUTTED (3), CLAIMED (4), IN CHECK (5), ACCEPTED (6), PAID (8), READY TO PAY (9), REJECTED (10) or ON HOLD (11)", () => {
          test.each([
            ...
            "Status is DATA INPUTTED (3), CLAIMED (4), IN CHECK (5), ACCEPTED (6), PAID (8), READY TO PAY (9), REJECTED (10) or ON HOLD (11)",
            async ({ latestApplications }) => {
              const SBI = 123456789;
              getLatestApplicationsBySbi.mockResolvedValueOnce(
                latestApplications
              ...
              await expect(businessEligibleToApply(SBI)).rejects.toEqual(
                new CannotReapplyTimeLimitError(
                  "Business with SBI 122333 is not eligible to apply due to 10 month restrictions since the last agreement."
                )
              );
            }
          );
    ...
    test("Last application and next application dates are returned with a CannotReapplyTimeLimitError", async () => {
      ...
      const expectedErrorData = {
        lastApplicationDate: "28 February 2024",
        nextApplicationDate: "29 December 2024",
      };
      const wrongExpectedErrorData = {
        lastApplicationDate: "28 January 2024",
        nextApplicationDate: "29 Januaary 2024",
      };
      getLatestApplicationsBySbi.mockResolvedValueOnce(apiResponse);
      ...
      expect(thrownError.lastApplicationDate).toEqual(
        expectedErrorData.lastApplicationDate
      );
      expect(thrownError.lastApplicationDate).not.toEqual(
        wrongExpectedErrorData.lastApplicationDate
      );
      expect(thrownError.nextApplicationDate).toEqual(
        expectedErrorData.nextApplicationDate
      );
      expect(thrownError.nextApplicationDate).not.toEqual(
        wrongExpectedErrorData.nextApplicationDate
      );
    });
  });
        ...
        describe("Business is not eligible when the last previous application had a status of anything other than WITHDRAWN (2), NOT AGREED (7), AGREED (1)", () => {
          describe("Already Applied error is returned with status of DATA INPUTTED (3), CLAIMED (4), IN CHECK (5), ACCEPTED (6), PAID (8), READY TO PAY (9) or REJECTED (10)", () => {
            test.each([
              ...
              "Status is DATA INPUTTED (3), CLAIMED (4), IN CHECK (5), ACCEPTED (6), PAID (8), READY TO PAY (9) or REJECTED (10)",
              async ({ latestApplications }) => {
                const SBI = 123456789;
                getLatestApplicationsBySbi.mockResolvedValueOnce(
                  latestApplications
                ...
                await expect(businessEligibleToApply(SBI)).rejects.toEqual(
                  new AlreadyAppliedError(
                    "Business with SBI 122333 is not eligible to apply"
                  )
                );
              }
            );
