...
jest.mock("../../../../app/api-requests/rpa-api/organisation", () => ({
  ...
  getOrganisationAddress: jest.fn(),
}));
jest.mock("../../../../app/api-requests/rpa-api/cph-numbers");
jest.mock("../../../../app/event/get-ineligibility-event", () => ({
  getIneligibilityEvent: jest.fn(),
}));
...
describe("FarmerApply defra ID redirection test", () => {
  ...
  describe(`GET requests to '${url}'`, () => {
    test.each([
      ...
      "returns 400 and login failed view when empty required query parameters - %p",
      async ({ code, state }) => {
    ...
    test("returns 302 and redirected to org view when authenticate successful", async () => {
      ...
      expect(setFarmerApplyData).toBeCalledWith(
        expect.anything(),
        "organisation",
