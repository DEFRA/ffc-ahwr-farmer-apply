...
When("I enter my valid {string}", async function (email) {
  ...
  await browser.pause(5000)
  let elem = await browser.$('#email')
  let elText = await elem.getText()
...
When("I should see an error {string}",async (email)=>{
  let errorText ="No user found with email address \""+email+"\"";
  let actualText = await FarmerLogin.errorField.getText()
  expect(errorText).to.equals(errorText)
})
