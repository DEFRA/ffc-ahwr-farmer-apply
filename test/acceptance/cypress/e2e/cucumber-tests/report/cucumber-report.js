...
const evnPlatform = process.platform;
const cucumberJsonDir = 'cypress/cucumber-report';
const cucumberReportFileMap = {};
const cucumberReportMap = {};
const jsonIndentLevel = 2;
const htmlReportDir = 'cypress/cucumber-report';
...
function addSnapshots() {
    ...
    failingFeatures.forEach((feature) => {
        if (fs.existsSync(path.join(snapshotDir, feature, '__diff_output__'))) {
            const snapshots = fs
              .readdirSync(path.join(snapshotDir, feature, '__diff_output__'))
              .filter((file) => {
                  return file.indexOf('.png') > -1;
              });
...
function generateReport() {
    if (!fs.existsSync(cucumberJsonDir)) {
        ...
        report.generate({
            ...
            reportName: `AHWR vet-visit acceptance test report - ${new Date().toLocaleString()}`,
            metadata: {
                browser: {
                    name: 'chrome',
                },
